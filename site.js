...
const translations = {
  zh: {
    ...
    text: {
      ...
      'vision p': '过去四十年，企业软件以项目为单位被一次次重写——需求来了，代码堆上去；团队走了，系统烂在那里。我们相信，AI 时代会有另一种活法：需求被 AI 解析成模型，模型铺开主干，定制代码延伸边界；模型在运行时生长为应用，应用在运营中沉淀为资产，资产再反哺下一次需求。潮汐栈构建的，正是让这个循环持续运转的基础设施。',
      ...
      'principles p': '它们决定潮汐栈如何做每一个技术决策，也决定系统如何被构建、引入和持续演化。',
      'principle-1-title': '资产持续沉淀',
      'principle-1-body': '每一次构建都应该留下可以继续使用的东西。模型、应用、规则、配置，以及为特定场景编写的扩展代码，都会成为可管理、可复用、可演进的资产，而不是随着项目结束一起失效。',
      'principle-2-title': '渐进式演化',
      'principle-2-body': '不要求一次性推倒重来。可以从一个模型、一个应用、一段流程，或一个业务领域开始，选择合适的边界逐步引入，让新旧系统在演化中衔接，建设随着价值自然扩展。',
      'principle-3-title': '架构清晰可组合',
      'principle-3-body': '模块边界清晰，职责单一，每个能力都可以被独立理解、验证和复用。新的需求通过组合和扩展完成，而不是不断把复杂度堆进一个系统。',
      'principle-4-title': '人机各司其职',
      'principle-4-body': '让 AI 处理理解、生成和重复性工作，让人把握目标、判断和授权。AI 可以加速行动，但关键决策、例外处理和最终责任，始终有清晰的归属。',
  ...
  en: {
    ...
    text: {
      ...
      'principles p': 'They shape how TideStack makes each technical decision, and how systems are built, introduced, and continuously evolved.',
      'principle-1-title': 'Assets keep accumulating',
      'principle-1-body': 'Every build should leave something that can keep being used. Models, applications, rules, configurations, and extensions written for a specific context become manageable, reusable, evolvable assets instead of expiring with the project.',
      'principle-2-title': 'Progressive evolution',
      'principle-2-body': 'No big-bang rewrite is required. Start with a model, an application, a workflow, or a business domain; introduce TideStack at the right boundary, let new and existing systems connect as they evolve, and expand as value proves out.',
      'principle-3-title': 'Architecture is clear and composable',
      'principle-3-body': 'Modules have clear boundaries and focused responsibilities. Each capability can be understood, verified, and reused independently; new requirements are met through composition and extension instead of accumulating complexity in one system.',
      'principle-4-title': 'Human and AI each do their part',
      'principle-4-body': 'Let AI handle understanding, generation, and repetitive work; let people set goals, exercise judgment, and grant authorization. AI accelerates action, while key decisions, exceptions, and final accountability always have a clear owner.',
...
function updateWaves() {
  ...
  boundaryWaves.forEach((item, index) => {
    const reveal = getWaveReveal(item.section, scrollTop);
    const transitionProgress = 1 - reveal;
    const direction = index % 2 === 0 ? 1 : -1;
    const pathProgress = transitionProgress;
    const flowProgress = reveal;
    const x = Math.sin(scrollProgress * Math.PI * 3 + index * 0.5) * 7 * transitionProgress * direction;
